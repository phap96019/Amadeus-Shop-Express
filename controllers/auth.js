  ...
  try {
    ...
    if (checkToken)
      return res
        ...
        .json({
          msg:
            "The login process has encountered an error, please log in again",
        });
...
exports.refreshToken = async (req, res) => {
  try {
    ...
    if (refreshToken == null)
      return res.sendStatus(401).json({ message: "Error refreshToken null" });
    ...
    if (!checkToken)
      return res
        .sendStatus(403)
        .json({ message: "The refreshToken has expired" });
    ...
    jwt.verify(checkToken.token, process.env.JWT_SECRET_REFRESH, err => {
      if (err)
        return res
          .sendStatus(403)
          .json({
            message: "Authentication error. Please check out the refresh token",
          });
